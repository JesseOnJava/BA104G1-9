		...
		if ("update".equals(action)) { // 來自update_newsdetail_input.jsp的請求
			...
			try {
				...
				if(!errorMsgs.isEmpty()){
					req.setAttribute("healthNewsDetailVO", healthNewsDetailVO); // 含有輸入格式錯誤的HealthNewsDetailVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back/healthnewsdetail/update_newsdetail_inout.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back/healthnewsdetail/update_newsdetail_inout.jsp");
				failureView.forward(req,res);
				System.out.println("*******************");
			}
		}
...
/************************************** 查詢單個 getOne_For_Display **********************************************************************************/		
		...
		if("getOne_For_Display_Front".equals(action)){
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)*************/
				req.setAttribute("healthNewsDetailVO", healthNewsDetailVO); // 資料庫取出的VO物件,存入req
				String url = "/front/healthnews/newsdetail.jsp";              
				RequestDispatcher successView = req.getRequestDispatcher(url); // 成功轉交listOneEmp.jsp
				successView.forward(req, res);
...
/**************************************  getOne_For_Update **********************************************************************************/		
		...
		if ("getOne_For_Update".equals(action)) { 
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("healthNewsDetailVO", healthNewsDetailVO); // 資料庫取出的VO物件,存入req
				String url = "/back/healthnewsdetail/update_newsdetail_input.jsp";
...
/************************************** delete **********************************************************************************/		
		...
		if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			try {
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back/healthnewsdetail/listAllNewsdetail.jsp");
				failureView.forward(req, res);
			}
		}
	}		
