...
//				// Send the use back to the form, if there were errors
		...
		if ("listHcWorks_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
			...
			try {
				...
				List<HcWorkShiftsVO> hcWorkShiftsVOList  = hcWorkShiftsService.getAll(map);
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.getSession().setAttribute("hcWorkShiftsVOList", hcWorkShiftsVOList); // 資料庫取出的list物件,存入request
